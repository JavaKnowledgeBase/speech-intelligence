...
class ProviderCatalog:
    ...
    def statuses() -> list[ProviderStatus]:
        ...
        return [
            ...
            ProviderStatus(provider="LiveKit", purpose="Realtime media transport", configured=settings.configured(settings.livekit_url) and settings.configured(settings.livekit_api_key), environment_key="LIVEKIT_URL/LIVEKIT_API_KEY", mode=live_mode, notes="Recommended for child-facing live sessions."),
            ...
            ProviderStatus(provider="Supabase", purpose="Clinical data, object storage, environment standards, curriculum targets, and vector state", configured=settings.configured(settings.supabase_url) and settings.configured(settings.supabase_service_role_key), environment_key="SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY", mode=live_mode, notes="Recommended as the operational data backbone and pgvector store."),
